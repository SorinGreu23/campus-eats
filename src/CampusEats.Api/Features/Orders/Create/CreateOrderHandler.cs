...
public class CreateOrderHandler : IRequestHandler<CreateOrderRequest, IResult>
{
    ...
    public async Task<IResult> Handle(CreateOrderRequest request, CancellationToken cancellationToken)
    {
        ...
        var menuItemIds = request.Items.Where(i => i.MenuItemId.HasValue).Select(i => i.MenuItemId!.Value).ToList();
        if (!menuItemIds.Any())
        ...
        if (!string.IsNullOrWhiteSpace(order.OrderType) && order.OrderType.Equals("Delivery", StringComparison.OrdinalIgnoreCase))
        {
        ...
        {
            if (!itemReq.MenuItemId.HasValue) continue;
